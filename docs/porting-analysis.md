...
| Partitions and source templates | Current project, pbi-tools behavior signal | Build early for dummy and handoff | First slice implemented: credential-free SQL source-template sidecars plus `handoff rebind-plan`. Excel/CSV/generic M and executable partition replacement remain planned. |
