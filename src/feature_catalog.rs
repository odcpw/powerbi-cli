...
const FEATURE_CATALOG: &[Feature] = &[
    ...
    Feature {
        ...
        refusal_code: None,
        reason: "powerbi-cli.dashboard.v2 is a strict superset of v1 with versioned allowed-key tables, bounded relative $include composition, and deny-unknown-fields models. `report spec normalize` flattens model, page, and style fragments deterministically; `report spec upgrade` losslessly rewrites every validated v1 spec to normalized v2, preserving array order and refusing unknown keys before writing. The compiled subset is artifact-identical to v1; proof is compiled into a side-effect-free proofPlan and exact next commands, while every other recognized future section stops with unsupported_feature and its owning T3 bead id.",
        next_proof: &[
