...
pub(crate) fn report_command(args: &[String]) -> CliResult<Value> {
    match args {
        ...
        [] => Err(CliError::invalid_args(
            "report requires a subcommand: build, spec fields, spec validate, spec upgrade, design-plan, wireframe export, pages, bookmarks, filters, slicers, interactions, themes, visuals",
        )
