//! MCP client session, protocol framing, closed tool policy, and identity validation.

use super::*;

const DEFAULT_FRAME_LIMIT: usize = 512 * 1024;
pub(super) const DEFAULT_TOTAL_RESPONSE_LIMIT: usize = 4 * 1024 * 1024;
const DEFAULT_STDERR_LIMIT: usize = 32 * 1024;
const DEFAULT_CALL_TIMEOUT: Duration = Duration::from_secs(30);
const DEFAULT_SESSION_TIMEOUT: Duration = Duration::from_secs(2 * 60);
pub(super) const DEFAULT_CLEANUP_TIMEOUT: Duration = Duration::from_secs(3);
...
impl McpFailure {
    ...
    pub(super) fn panicked(message: impl Into<String>) -> Self {
        Self::new(McpFailureKind::Panicked, message)
    }

    pub(super) fn new(kind: McpFailureKind, message: impl Into<String>) -> Self {
        Self {
            kind,
    ...
    pub(super) fn with_cleanup(mut self, cleanup: &McpCleanupReport) -> Self {
...
impl McpSession {
    ...
    pub(super) fn remaining_call_timeout(&self) -> Result<Duration, McpFailure> {
...
pub(super) fn checked_identifier(value: &str, label: &str) -> Result<String, McpFailure> {
...
pub(super) fn redact_vendor_text(text: &str) -> String {
