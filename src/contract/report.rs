...
pub(super) fn commands() -> Vec<Value> {
    vec![
        ...
        json!({
            ...
            "styleCompilation": "style.preset compiles to ApplyThemePreset and style.bundle to ApplyStyleBundle at the final operation stage. Literal-text bundles require style.allowLiteralText:true. style.tokens compile to registered themes and number formats; style.defaults compile catalog-backed per-visual formatting.",
        ...
        json!({
            ...
            "followUpFields": ["ok", "exitCode", "validationLevel", "normalizedFrom", "compiled.counts", "compiled.defaultsApplied", "defaultsApplied", "proofPlan.requestedLevel", "proofPlan.achievableHere", "proofPlan.commands[]", "proofPlan.unavailable[].what", "proofPlan.unavailable[].why", "proofPlan.unavailable[].whereItWorks", "warnings", "errors", "errors[].code", "errors[].message", "errors[].path", "errors[].pointer", "errors[].field", "errors[].reason", "errors[].candidatesCommand", "errors[].example", "next"],
            "validationLevels": [
            ...
            "diagnosticCodes": ["spec.missing_input", "spec.unknown_field", "unsupported_feature", "invalid_args", "include.invalid", "include.parse", "include.path_escape", "include.cycle", "include.unsupported_location", "input_safety_violation"],
        ...
        json!({
            ...
            "usage": "powerbi-cli report plan --schema <schema.json> [--profile <profile.json>] [--project <project-dir-or.pbip>] (--intent <intent.md|intent.json> | --objective <goal>) [--out <dashboard.json>] [--variants <N>] [--explain-rules] --json",
            ...
            "flags": ["--schema <schema.json>", "--profile <profile.json>", "--project <project-dir-or.pbip>", "--intent <intent.md|intent.json|text>", "--objective <goal>", "--out <dashboard.json>", "--variants <N>", "--force", "--explain-rules", "--json", "--format json"],
            "variants": {"count":"1..8; requires --out; insufficient compatible choices refuse without writing", "paths":"<out>.variant-<i>.json, indices start at 1", "ordering":"catalog score descending, structure hash ascending", "outputFields":["index", "path", "structureHash", "score", "decisionDiff", "validationLevel", "validateCommand", "performance", "guardDecisions", "guardApplication"], "validation":"compiler plus schema validation before publishing; Linux proof only", "guards":"separate replay proposals, not embedded in validated candidate specs"},
            "examples": [
