...
define_rules! {
    ...
    DESIGN_VISUAL_OFF_GRID => ("design.visual_off_grid", Design, "warning", "A visual edge is not aligned to the twelve-column design grid.", "Run `report layout auto --snap` to move edges onto the nearest column guides while keeping the arrangement, or re-run `report layout auto` with a named template when a slot layout fits.", Some("relayout-template")),
