...
const FEATURE_CATALOG: &[Feature] = &[
    ...
    Feature {
        ...
        refusal_code: None,
        reason: "Every native validation, lint, and report-audit finding id is registered with family, severity, summary, remediation, optional sanitize action, and version metadata; validation and design findings carry their source file and RFC 6901 pointer, while agents can list or explain rules without opening a project. M lint includes the error-level m.duplicate_step_name check for duplicate let identifiers.",
        next_proof: &[],
        reference_signals: &[],
    ...
    Feature {
        ...
        reference_signals: &[
            "src/design/lint.rs",
            "src/design/templates.json",
            "docs/bridge-plan-2026-09.md:T5.3",
        ],
        tags: &["lint", "design", "geometry", "grid", "scorecard", "agent"],
    },
    Feature {
        id: "quality.model-completeness-lint",
        title: "DAX format and semantic-model completeness lint",
        category: "validation",
        status: "supported",
        support: "offline-static-heuristics",
        proof_level: "unit-smoke",
        emits_pbir: false,
        commands: &["lint", "triage", "model dax lint"],
        refusal_code: None,
        reason: "Static lint reports measures without explicit display formats, malformed custom format strings, visible relationship keys, suspicious both-direction fact-to-dimension relationships, and model columns with no visual, measure, or relationship references. Every warning includes a stable handle and a concrete remediation hint.",
        next_proof: &[
            "Run the same project through Power BI Desktop and review formatting and filter direction before claiming compatibility",
        ],
        reference_signals: &["src/model_dax.rs", "docs/bridge-plan-2026-09.md:T10.5"],
        tags: &[
            "lint",
            "dax",
            "format-string",
    ...
    Feature {
        id: "desktop.refresh-check",
        title: "Desktop refresh proof",
        category: "desktop",
        status: "planned",
        support: "planned-windows-oracle",
        proof_level: "unit-smoke",
        emits_pbir: false,
        commands: &["desktop refresh-check"],
        refusal_code: Some("unsupported_feature"),
        reason: "T9.1 will drive a managed Windows Desktop refresh and collect per-table errors before a desktop-canvas-refresh claim. The proof planner advertises the exact command now, but this release does not execute it.",
        next_proof: &[
            "Implement T9.1 refresh automation on an explicitly opted-in Windows Desktop oracle",
        ],
        reference_signals: &["docs/bridge-plan-2026-09.md: T9.1 desktop refresh-check"],
        tags: &["desktop", "oracle", "refresh", "proof", "planned"],
    },
    Feature {
        id: "desktop.canvas-check",
        title: "Desktop canvas and expected-value proof",
        category: "desktop",
        status: "planned",
        support: "planned-windows-oracle",
        proof_level: "unit-smoke",
        emits_pbir: false,
        commands: &["desktop canvas-check"],
        refusal_code: Some("unsupported_feature"),
        reason: "T9.2 will assert proof.expectValues through the exact Desktop model and reject blank canvases after refresh. The proof planner advertises the exact command now, but this release does not execute it.",
        next_proof: &[
            "Implement T9.2 canvas and expected-value automation on an explicitly opted-in Windows Desktop oracle",
        ],
        reference_signals: &["docs/bridge-plan-2026-09.md: T9.2 desktop canvas-check"],
        tags: &["desktop", "oracle", "canvas", "refresh", "proof", "planned"],
    },
    Feature {
    ...
    Feature {
        id: "desktop.reference-harvest",
        title: "Desktop-authored PBIR reference harvesting",
        category: "desktop",
        status: "supported",
        support: "linux-safe-provenance-archive",
        proof_level: "desktop-golden-pending",
        emits_pbir: false,
        commands: &["desktop harvest-reference"],
        refusal_code: None,
        reason: "The CLI can archive a visual.json, page.json, or report.json fragment from an already-saved PBIP project after the shared harvested-fragment safety reader rejects persisted selection/filter values. The archive records a source-project SHA-256 fingerprint, source path, date, license note, and Desktop version when supplied, while keeping the compatibility claim at desktop-golden-pending.",
        next_proof: &[
            "Capture Desktop save/version evidence on Windows and attach it to the archived reference",
            "Promote a reference only after a generated artifact completes the Desktop canvas, refresh, save, and reopen oracle",
        ],
        reference_signals: &[
            "docs/bridge-plan-2026-09.md: T9.7 fixture harvester",
            "docs/reference/desktop-authored-visuals/NOTES.md: Microsoft BCApps attribution and persisted slicer-state warning",
            "src/input_safety.rs: read_harvested_fragment refusal contract",
        ],
        tags: &[
            "desktop",
            "oracle",
    ...
    Feature {
        id: "model.tables",
        title: "Semantic-model table inventory and CRUD",
        category: "model",
        status: "supported",
        support: "read-write",
        proof_level: "unit-smoke",
        ...
        refusal_code: None,
        reason: "The CLI parses table TMDL documents, emits typed offline-safe tables with dummy partitions, and performs guarded rename/delete operations while refusing unresolved references or unknown table-level metadata.",
        next_proof: &[
            "Desktop open/save fixture for table-level metadata and table rename compatibility",
        ],
        reference_signals: &[],
        tags: &["table", "semantic-model", "tmdl", "crud", "handles"],
    },
    Feature {
        id: "model.columns",
        title: "Semantic-model base and calculated column inventory and CRUD",
        category: "model",
        status: "supported",
        support: "read-write",
        proof_level: "unit-smoke",
        ...
        refusal_code: None,
        reason: "The CLI parses base and calculated column blocks and rewrites only modeled properties; unknown Desktop-authored properties in an updated block cause an explicit refusal so they are never dropped.",
        next_proof: &[
            "Desktop refresh fixture for typed base-column updates and calculated-column recomputation",
        ],
        reference_signals: &[],
        tags: &["column", "semantic-model", "tmdl", "crud", "handles"],
    },
    Feature {
        id: "model.calculated-tables",
        title: "Calculated semantic-model tables",
        category: "model",
        status: "supported",
        support: "read-write",
        proof_level: "unit-smoke",
        emits_pbir: false,
        commands: &["model tables add-calculated"],
        refusal_code: None,
        reason: "The CLI emits a real TMDL table with a calculated DAX partition, validates credential-free input, and exposes the partition to the existing offline DAX dependency and lint analysis. Model completeness defers the no-columns check until Desktop materializes the schema. Power BI Desktop remains the engine and schema-materialization oracle.",
        next_proof: &[
            "Desktop open/save and refresh fixture proving calculated-table schema materialization",
        ],
        reference_signals: &[],
        tags: &[
            "table",
    ...
    Feature {
        ...
        refusal_code: None,
        reason: "The CLI extracts common DAX references from measures, calculated columns, and calculated-table partitions and reports obvious dependency defects while explicitly reporting that Desktop/XMLA remains the DAX engine validation boundary.",
        next_proof: &[
    ...
    Feature {
        ...
        refusal_code: None,
        reason: "Roles/RLS, perspectives, and cultures/translations can be inventoried from TMDL source files; named expressions have a separate guarded read-write authoring feature.",
        next_proof: &[
            "Desktop-authored TMDL fixtures for role, perspective, and culture mutations",
        ],
        reference_signals: &[
    ...
    Feature {
        id: "model.named-expressions",
        title: "Named M expression authoring",
        category: "model",
        status: "supported",
        support: "read-write",
        proof_level: "unit-smoke",
        ...
        refusal_code: None,
        reason: "The CLI adds, updates, and deletes named expression blocks with bounded UTF-8 M input, newline-preserving TMDL edits, duplicate-step lint coverage, guarded in-place deletion, and fail-closed handling for unknown Desktop-authored metadata.",
        next_proof: &[
            "Desktop refresh fixture for named-expression dependency and query-folding behavior",
        ],
        reference_signals: &[
            "https://learn.microsoft.com/en-us/power-bi/transform-model/desktop-using-parameters",
        ],
        tags: &[
            "tmdl",
            "semantic-model",
    ...
    Feature {
        id: "ops.atomic-replay",
        title: "Atomic typed operation replay",
        category: "workflow",
        status: "supported",
        support: "typed-plan-transaction",
        proof_level: "unit-smoke",
        emits_pbir: true,
        commands: &["ops apply"],
        refusal_code: None,
        reason: "Bounded ops.v1 plans use the registered kernels and validate the staged result before an atomic commit. Dry-run scores the staged tree; in-place mutations retain a sibling snapshot. Raw patches and operation-embedded output controls are refused.",
        next_proof: &[],
        reference_signals: &["tests/ops_apply.rs"],
        tags: &["ops", "replay", "atomic", "validation"],
    },
    Feature {
    ...
    Feature {
        id: "report.compose",
        title: "One-command report composition",
        category: "report",
        status: "supported",
        support: "in-process-narrative-pipeline",
        proof_level: "unit-smoke",
        emits_pbir: true,
        commands: &["report compose"],
        refusal_code: None,
        reason: "Composes existing profile inference, narrative planner, spec validation/explain, build, triage and wireframe stages without self-execution. Publishes a validated project plus intermediate sidecars; dry-run publishes neither, confirmed in-place uses snapshot transactions. Native unsupported compiler sections and missing evidence remain refusals.",
        next_proof: &["Desktop canvas and refresh proof for composed projects"],
        reference_signals: &["tests/report_compose.rs"],
        tags: &["report", "compose", "agent"],
    },
    Feature {
    ...
    Feature {
        ...
        next_proof: &["Desktop canvas fixture for explicit interaction UI state"],
        reference_signals: &[
    ...
    Feature {
        ...
        category: "report",
        status: "supported",
        support: "read-write-reset-to-default",
        proof_level: "unit-smoke",
        emits_pbir: true,
        commands: &["report interactions reset"],
        refusal_code: None,
        reason: "Reset removes one explicit visualInteractions row after resolving live page/source/target handles; the resulting absent row documents that Power BI applies the target visual's default interaction behavior. Desktop canvas confirmation remains pending.",
        next_proof: &[
            "Capture Desktop reset/default interaction fixtures",
            "Open, render, save, and reopen a reset report in Power BI Desktop",
        ],
        reference_signals: &[
            "https://developer.microsoft.com/json-schemas/fabric/item/report/definition/page/2.0.0/schema.json: page visualInteractions shape",
