...
Compose a first project through the existing planner and compiler in one call:

```bash
powerbi-cli report compose --schema examples/sales.schema.json --objective "Sales overview" --out-dir build/composed-sales --json
```

Optional `--rows` supplies bounded CSV/JSON for aggregate-only profiling;
`--profile` reuses a profile instead. Add `--intent` and `--style <preset|tokens.json>`
for explicit intent and style (for example, the `corporate-neutral` token preset).
Compose returns stage timings, scorecard, proof
plan, and SVG wireframes, and saves intermediates in `<out-dir>-compose/`.
Use `--dry-run` for an unpublished preview. Existing projects require
`--project ... --in-place --confirm <confirmToken>` from a project dry-run;
the snapshot transaction preserves recovery bytes. Output and sidecar targets
must be new; `--artifacts-dir` selects a fresh sidecar for recomposition.
Native stage refusals are preserved; write-mode failures retain completed
sidecars for recovery. Stage timing fields vary; generated project bytes do not.
The alias is `dashboard new`; only the default template set and one variant
...
- `powerbi-cli report layout auto --project <project-dir-or.pbip> [--page <page-name-or-handle>] [--template <name> | --preset overview|analysis|detail|grid | --snap] [--page-size 1280x720|1920x1080] [--grid columns=12,gutter=16,margin=24,rowUnit=8] [--margin <n>] [--gap <n>] [--row-unit <n>] (--dry-run | --in-place | --out-dir <dir>) --json` — Resolve named twelve-column design-system slots and reposition existing visuals into deterministic canvas coordinates without changing bindings or formatting; legacy overview|analysis|detail|grid presets are aliases for the corresponding named templates, and --snap keeps the existing arrangement while moving each visual's edges onto the nearest column start/end guides and row-unit multiples, reporting any emergent design.visual_overlap in warnings _(proof: `unit-smoke`)_
...
`performance.ops` SetTopNGuard operations. JSON intent can override these
catalog defaults with `"guards": {"threshold": 200, "top": 50}`.
Decisions record counts, threshold, cap, shape, and ranking measure; the
first bound measure is reused, or a cheap COUNTROWS measure is proposed for
one unambiguous fact table. The legacy `spec` / `--out` remains unchanged;
these operations target the v2 candidate and do not imply full v2 build support.
`performance.kernelAvailable` reports typed replay availability, and
`performance.previewCommands` provides the existing TopN mutation dry-run path.
Optional `--project <project-dir-or.pbip>` inspects existing partition M with
the registered `m.unbuffered_reuse` analyzer and recommends reviewing
Table.Buffer and query folding, without rewriting partitions. Without a
project, `performance.bufferAnalysis` reports `project-not-supplied`.
...
- Programmatic visual authoring currently covers first-slice PBIR visual
  behavior in dashboard v2: `drilldown.fields` compiles hierarchy columns,
  `sort` supports one descending projected measure on combo/pie/donut, and
  `topnGuard` ranks the first hierarchy/Category column by `orderBy` with a
  positive `top`. Visual `filters[]` use the visual-scoped AddFilter kernel.
  These share the CLI mutation kernels; unproven sorts remain fixture-gated.
  The visual authoring surface also covers
