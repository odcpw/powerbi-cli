...
pbi report audit --project build/sales --rules design --json
...
pbi report visuals set-object --project build/sales --batch formatting.ops.json --dry-run --json
...
The report design-system surface also exposes the exact command paths
`report style tokens show` and `report style tokens derive`. The embedded
`powerbi-cli.tokens.v1` catalog has
corporate-neutral, high-contrast, dark, and print sets; `style.tokens` builds a
registered theme with semantic colors, ramps, text classes, and visual defaults,
maps locale-aware numeric formats and label display units into TMDL/theme metadata, and refuses foreground/page pairs below WCAG AA
unless `allowContrastBelowAA: true` records a warning and handoff note. Derive
samples formatting colors from a report and never copies literal titles or names.
...
`performance.ops` SetTopNGuard operations with numeric decisions. These target
the v2 candidate; the legacy `spec` and `--out` stay build-compatible.
Inspect `performance.kernelAvailable` and use `performance.previewCommands`
for the existing TopN mutation dry-run path while kernel integration is pending.
Use optional `--project <project-dir-or.pbip>` for existing partition
`m.unbuffered_reuse` findings and Table.Buffer review recommendations;
...
severity, the available design-geometry lint report, offline handoff status,
and the honest proof level. Pass `--trace` when diagnosing a build to include the
