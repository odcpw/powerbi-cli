...
| Visual interactions | `pbir-utils` | Readback plus guarded explicit override and reset slices implemented | `report interactions list/show` inventories explicit page `visualInteractions`, resolves source/target handles, and flags stale references. `report interactions set/disable` upserts DataFilter, HighlightFilter, or NoFilter rows for live source/target visual pairs; `report interactions reset` removes one matching row and documents that absence restores the target default. Desktop-authored canvas confirmation remains pending. |
...
| Lint/rules | `pbir-utils`, Tabular Editor BPA | Build early | First BPA-lite slice implemented: DAX static findings, duplicate page/visual titles, and validator-rejected `general.altText` placements with cleanup guidance. Broader rule packs should catch overfull pages, hidden unused objects, broken refs, unsafe files, and style drift. |
...
- `report interactions list/show/set/disable/reset` (implemented; reset removes
  one explicit row and records the documented default semantics; Desktop canvas
