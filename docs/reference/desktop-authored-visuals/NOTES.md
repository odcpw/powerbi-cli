...
New references captured from an already-saved PBIP should be written with
`desktop harvest-reference`. The command stores a `powerbi-cli.desktop-reference.v1`
wrapper with the source fragment under `fragment` and a `provenance` object
containing `sourcePath`, `sourceProject`, `sourceFingerprint`, `date`,
`desktopVersion`, and `licenseNote`. The selected fragment passes the shared
input-safety reader; persisted slicer/filter selections are refused rather than
silently copied or stripped. Linux captures use `desktopVersion: "unknown"` and
remain `desktop-golden-pending` until Desktop evidence is attached.
